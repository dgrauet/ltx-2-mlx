...
| `--video-decoder {conv,diffusion}` | conv | LTX 2.5 diffusion video decoder (sharper, slower; tiled automatically above `LTX2_VAE_DECODE_BUDGET_GB`). Border numerics follow upstream's default chunked mode. | `generate` (all variants) |
...
| `LTX2_VAE_DECODE_BUDGET_GB=N` | RAM÷2 | Peak-memory budget of the VAE decode (both decoders); drives the conv decoder's temporal-then-spatial auto tiling (~750 B per output pixel-frame, measured) and the diffusion decoder's tile sizing. Raise on 64-256 GB machines for fewer, larger tiles. | video decode |
| `LTX2_VAE_DECODE_KEEP_CACHE=1` | unset | Keep MLX's allocator cache during the VAE decode. By default the cache is disabled for the decode only (pixels identical, process footprint tens of GB lower on long clips). | video decode |
