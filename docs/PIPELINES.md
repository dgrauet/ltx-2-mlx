...
| `--stepwise-image-output-dir DIR` | off | *(experimental)* Decode one latent frame of the in-progress x0 prediction to a PNG every N steps, plus a growing `*_progress.webp` you can open mid-run. Costs ≈ one full decode ÷ latent frame count per preview, and keeps the VAE decoder resident through denoising (so it works against `--low-ram`, which warns). Multi-stage pipelines tag files `_s1` / `_s2`. | all generating pipelines |
| `--stepwise-interval N` | 1 | Preview every N denoising steps. The final step is always previewed. | with `--stepwise-image-output-dir` |
| `--stepwise-frame I` | middle | Latent frame to preview. Negative counts from the end (`-1` = last). Frame 0 is the clean conditioning image on I2V runs. | with `--stepwise-image-output-dir` |
