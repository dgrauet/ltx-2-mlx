...
| `--diffvae-tile FRAMES HEIGHT WIDTH` | auto | Diffusion decoder tile size in pixel frames / pixels (multiples of 8 / 32, 0 = axis untiled, `0 0 0` = force one tile under `LTX2_DIFFVAE_MAX_TOKENS`). Overlaps are fixed at 40 frames / 160 px. | with `--video-decoder diffusion` |
